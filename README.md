...
スタッフの入口は`/`、注文は`/sales`、調理は`/kitchen`、受け渡しは`/handoff`である。入口の管理者ページセクションから、スタッフ管理の`/staff-management`と在庫管理の`/inventory-management`へ直接移動する。未認証で注文・調理・受け渡し・管理画面へアクセスすると`/`へ戻り、ログインを案内する。公開ホストの`/staff`以下は、`/staff`の接頭辞を除いてスタッフホストへ302転送する。
