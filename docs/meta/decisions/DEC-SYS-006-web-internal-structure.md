...
`widgets/layout`を一つのスライスとし、ヘッダー、フッター、ナビゲーション、アカウント操作の接続を内部の部品として持つ。ナビゲーションの一覧とロール別の表示条件は`model/navigation.ts`に置く。スタッフページも同じスライスの公開入口からナビゲーションとアカウント欄を読む。

アカウントカードの表示は`entities/staff/ui`に置き、操作はコールバックで受け取る。`widgets/layout/ui/account.tsx`が既存の認証操作に接続し、ログアウトの処理中・失敗表示を担当する。ログアウト成功時は通信を取り消し、認証キャッシュを未ログインへ更新し、その他のキャッシュを削除してセッションを再確認する。共通レイアウトはページを参照せず、本文を`children`で受け取る。
...
`src/routes/`はTanStack Routerのファイル経路であり、`pages`の画面と`widgets/layout`の共通レイアウトを結線する。ルート経路で`Layout`へ`Outlet`を渡し、全画面へヘッダーとフッターを適用する。経路ファイルへ部品の見た目とデータ取得を書かない。経路一覧(`src/routeTree.gen.ts`)は生成物であり、直接編集しない。管理・会計・調理・受け渡しの経路は共通の認証レイアウトを通し、未認証なら`/`へ戻す。認証確認の通信失敗時はその場で再試行し、認証済み利用者のロール判定は各画面とAPIで行う。
