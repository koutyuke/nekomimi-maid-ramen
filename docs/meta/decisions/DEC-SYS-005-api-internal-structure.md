...
業務規則・状態遷移・計算はドメイン、必要なデータの取得と判定・保存・通知の順序はユースケースが所有する。RepositoryとCommandは読み書き、データ変換、原子的な保存、保存時の条件再確認を担う。操作の成立条件を保存実装だけに置かない。

| 操作         | ユースケース・ドメインで確認する条件                  | 保存時に保証すること                                                       |
| ------------ | ----------------------------------------------------- | -------------------------------------------------------------------------- |
| 在庫修正     | Admin以上の権限、商品存在                             | 最新権限と商品存在を再確認し、現在数の上書き・履歴・リビジョンを一括保存   |
| 調理状況更新 | 未取消・未受け渡し、対象明細存在、許可された状態遷移  | 読み出した明細状態・親注文状態・最新権限を更新条件に含める                 |
| 受け渡し完了 | 未取消・未受け渡し、明細存在、全明細完成              | 同じ条件と最新権限を再確認し、日時とリビジョンを一度だけ記録               |
| 注文取消     | 未取消・未受け渡し、完成明細なし                      | 同じ条件と最新権限を再確認し、取消・全明細の在庫復元・リビジョンを一括保存 |
| ロール変更   | 実行者の権限、自己変更・Owner保護、変更前後の許可範囲 | 最新ロールで再確認し、条件不成立なら変更しない                             |

在庫修正は`StockRepository.adjust`が在庫と履歴を原子的に保存する。履歴の修正前数量は保存直前の実際の数量を記録し、在庫未登録なら0とする。ユースケースの事前読み取り値を履歴へ転記しない。商品不存在と保存時の条件不成立は`PersistenceError`として扱う。

ポートには、入力と成功値に加え、条件不成立の結果、原子性、競合時の保証を記述する。注文確定の`DuplicateConfirmation`後は同じ要求識別子の既存注文を取得できる。`ConfirmationLostStockRace`後に問い合わせる不足情報は、その問い合わせ時点の情報である。再取得と業務上の結果の扱いはユースケースに置き、不足が解消していれば`OrderConfirmationConflict`を返す。

業務判断のテストは保存先を起動せず、ポートから事実と保存結果だけを返して検証する。保存側では原子性、状態の競合、事前確認後の権限剥奪を検証する。保存が成功した後だけ通知を要求する。
...
`public.ts`は自機能の契約と型を公開し、内部のコマンドやリポジトリなど永続化の詳細、`layer.ts`、テスト用コードを参照しない。`layer.ts`は自機能の`adapters`、`infra`、`application`、`domain`、`public.ts`を組み立て、他機能の`layer.ts`を参照しない。複数の表を一つの操作として更新するCommandは、その操作を所有する機能の`infra/commands`へ置き、自機能の内部ポートとドメイン、および`core/infra`を参照する。`routes`は機能の`public.ts`、`core/domain`、`core/adapters`を参照し、テストファイルでは`testing/index.ts`を利用できる。
