...
export const makeStockRepositoryLive = (ownerEmail: string) =>
  Layer.effect(
    StockRepository,
    Effect.gen(function* () {
                      ...
                      where ${menuItems.id} = ${menuItemId}
                        and exists (
                          ...
                          where ${users.id} = ${actorId}
                            and (${users.role} = 'Admin' or ${users.email} = ${ownerEmail})
                        )`,
                  )
                  .returning(),
                db
...
export const StockRepositoryLive = makeStockRepositoryLive("");
