...
export const UpdateCookingStateCommandLive = Layer.effect(
    ...
    return UpdateCookingStateCommand.of({
      execute: (actorId, id, menuItemId, from, to) =>
        database
                    ...
                    db
                      ...
                      .where(
                        and(
                          ...
                          or(eq(usersTable.role, "Staff"), eq(usersTable.role, "Admin"), eq(usersTable.role, "Owner")),
                        ),
                      ),
                  ),
                ),
          ...
          .pipe(Effect.flatMap((rows) => (rows.length === 1 ? Effect.void : Effect.fail(new KitchenOrderConflict())))),
    });
  }),
);
