...
import { CookingState, cookingStateOf } from "../../../core/domain/cooking-state";
...
export { CookingState } from "../../../core/domain/cooking-state";
...
export class Order extends Schema.Class<Order>("Order")({
  ...
  get cookingState(): CookingState {
    return cookingStateOf(this.lines);
  }
}

export class OperationalOrder extends Schema.Class<OperationalOrder>("OperationalOrder")({
  id: OrderId,
  businessDate: BusinessDate,
  orderNumber: OrderNumber,
  cancelledAt: Schema.NullOr(Schema.DateFromSelf),
  handedOffAt: Schema.NullOr(Schema.DateFromSelf),
  confirmedAt: Schema.DateFromSelf,
  lines: Schema.Array(
    Schema.Struct({
      menuItemId: MenuItemId,
